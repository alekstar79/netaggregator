<template>
    <div class="tool-wrapper right" v-bind="$attrs" v-click-outside="hide" :class="{ [color]: true, visible, flat: !docked }" :style="style">
        <div v-if="!mobile" class="drag-header-right" :class="{ [color]: true }">
        ...
        <nav class="right-toolbar">
            <template v-for="(tool, i) in tools">
                <v-tooltip :key="i" :disabled="mobile" left>
        ...
        <lazy-graph-new @custom="openCustomDialogue" @choose="$emit('create', $event)" />

        <v-dialog v-if="mobile" v-model="search" max-width="100%" :fullscreen="true">
            <lazy-graph-search @choose="$emit('image', $event)" @close="search = false" close />
        </v-dialog>
        <v-menu v-else
            v-model="search"
            :position-y="top"
            :position-x="window.width - 320"
            transition="slide-x-transition"
            content-class="pixabay-menu"
            max-width="315px"
            min-width="315px"
            absolute
            offset-x
        >
            <lazy-graph-search @choose="$emit('image', $event)" />
        </v-menu>

        <v-dialog content-class="custom-dialog" v-model="custom" :max-width="mobile ? '100%' : '540px'" :fullscreen="mobile" persistent>
            <lazy-graph-custom-dialogue :size="customProps" @create="createCustomCanvas" @close="custom = false" />
        </v-dialog>
    </div>
</template>
...
<script>
    ...
    import core from '~/mixins/canvas/core'
    ...
    export default {
        ...
        computed: {
            ...
            style() {
                return this.docked ? { right: this.right } : { zIndex: this.zIndex, ...this.flat }
            },
            window() {
                return this.$store.state.app.window
            },
            top() {
                const panelHeight = .88 * this.window.height

                return (this.window.height - panelHeight) / 2
            },
            mobile() {
        ...
        watch: {
            ...
            docked(value) {
                if (value) {
                    this.flat = { right: this.right, top: '50%' }
                }
            }
        },
        methods: {
            measure()
            {
                ...
                if ((h = this.$el.clientHeight)) {
                    this.$emit('change', { position: 'right', size: `${h}px` })
                }
            ...
            mousedown(e)
            {
                if (this.down || !whichBtn(e) || !check(e.target, 'drag-header-right')) return

                let rect = this.$el.getBoundingClientRect(),
                    { width: sw, height: sh } = this.$screen,
                    { move, end } = this.eventsDetermine,

                    shiftX = rect.left + rect.width - e.clientX,
            ...
            hide({ target })
            {
                ...
                this.$emit('hide', { position: 'right', state: false })
            },
            openCustomDialogue($event)
            {
                this.customProps = $event
                this.custom = true
            },
            openSearchDialog()
            {
        ...
        beforeDestroy()
        {
            ...
            this.$bus.$off('open:search-dialog', this.openSearchDialog)
        },
        beforeMount()
        {
            this.$bus.$on('open:search-dialog', this.openSearchDialog)
        },
        mounted()
        {
            this.update()
