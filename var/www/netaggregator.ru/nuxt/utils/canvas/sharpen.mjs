...
{
    const { convolutionFilter } = await import(/* webpackChunkName: "effect" */ './libs/effect/index.mjs'),
    ...
    function doit(img, f)
    {
        return convolutionFilter(img, 3, 3, [
            -f / 16, -f / 8, -f / 16,
            -f / 8, f * .75 + 1, -f / 8,
            -f / 16, -f / 8, -f / 16
        ])
    ...
    function sharpen({ canvas, src: a, dump }, f)
    {
    ...
    const Sharp = fabric.util.createClass(fabric.BaseBrush, {
        type: 'SharpenBrush',
        ...
        async setTargetObject(obj)
        {
            ...
            sharpen(this).then(this._setPattern.bind(this))
        ...
        setFactor(v)
        {
            if (this.src instanceof HTMLCanvasElement) {
                sharpen(this, v).then(this._setPattern.bind(this))
            }
        },
    ...
    fabric.sharpenTool = function(context) {
        const instance = new Sharp(context.canvas)
