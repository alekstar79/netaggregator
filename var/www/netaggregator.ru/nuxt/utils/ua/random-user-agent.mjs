...
const constructCumulativeWeightIndexPairsFromFilters = (filters) => {
    if (!filters) return defaultCumulativeWeightIndexPairs
...
* @see https://github.com/intoli/user-agents
*/
export class RandomUserAgent extends Function
{
    constructor(filters)
    {
        super()
    ...
    static random = (filters) => {
        try {
            return new RandomUserAgent(filters)
        } catch (error) {
            return null
        }
    };
    ...
    random = () => {
        const userAgent = new RandomUserAgent()
    ...
    randomize = () => {
        ...
        const [, index] = this.cumulativeWeightIndexPairs.find(([cumulativeWeight]) => cumulativeWeight > randomNumber)
