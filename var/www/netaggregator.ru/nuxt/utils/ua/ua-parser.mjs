// noinspection RegExpUnnecessaryNonCapturingGroup,RegExpRedundantEscape,RegExpSingleCharAlternation,JSUnfilteredForInLoop
...
has = function(str1, str2) {
    return typeof str1 === STR_TYPE
        ? lowerize(str2).includes(lowerize(str1))
        : false
},
...
const regexes = {
    ...
    device: [[
        ...
        // Sony
        /droid.+ ([c-g]\d{4}|so[-l]\w+|xq-a\w[4-7][12])(?= bui|\).+chrome\/(?![1-6]?\d\.))/i
...
const UaParser = function(ua, extensions) {
    ...
    let _ua = ua || ((typeof window !== UNDEF_TYPE && window.navigator && window.navigator.userAgent) ? window.navigator.userAgent : EMPTY)
