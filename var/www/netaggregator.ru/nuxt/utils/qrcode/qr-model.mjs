...
export function QrModel(typeNumber, errorCorrectLevel)
{
...
QrModel.prototype = {
    addData(data, binary) {
        let newData = new QR8bitByte(data, binary)
    ...
    makeImpl(test, maskPattern) {
        ...
        if (this.dataCache == null) {
            this.dataCache = QrModel.createData(this.typeNumber, this.errorCorrectLevel, this.dataList)
        }
    ...
    setupTimingPattern() {
        for (let r = 8; r < this.moduleCount - 8; r++) {
            if (this.modules[r][6][0] != null) continue

            this.modules[r][6][0] = (r % 2 === 0)
        }
        for (let c = 8; c < this.moduleCount - 8; c++) {
            if (this.modules[6][c][0] != null) continue
    ...
    setupTypeNumber(test) {
        ...
        for (let i = 0; i < 18; i++) {
            this.modules[Math.floor(i / 3)][i % 3 + this.moduleCount - 8 - 3][0] = (!test && ((bits >> i) & 1) === 1)
        }
        for (let i = 0; i < 18; i++) {
            this.modules[i % 3 + this.moduleCount - 8 - 3][Math.floor(i / 3)][0] = (!test && ((bits >> i) & 1) === 1)
        }
    },
...
QrModel.createData = function(typeNumber, errorCorrectLevel, dataList) {
    ...
    while (true) {
        ...
        buffer.put(QrModel.PAD0, 8)

        if (buffer.getLengthInBits() >= totalDataCount * 8) break

        buffer.put(QrModel.PAD1, 8)
    }
...
QrModel.createBytes = function(buffer, rsBlocks) {
    ...
    for (let r = 0; r < rsBlocks.length; r++) {
        ...
        for (let modIndex, i = 0; i < ecdata[r].length; i++) {
