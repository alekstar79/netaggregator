...
$cmd = buildCommandLine([$inkscapeCommand, '--shell']) . ' < ' . escapeArgument(turnSlashes($commandFile));
printf("Exec: %s\n", $cmd);
...
function exportSvg($path, $tmpFile, $forceUpdate): array
{
    $tmpPath = dirname($path) . '/' . $tmpFile;
    $outPath = preg_replace('/\.svg$/i', '', $path) . '.png';

    if (!$forceUpdate && file_exists($outPath) && filemtime($outPath) >= filemtime($path)) {
...
function buildCommandLine(array $args): string
{
    return implode(' ', array_map('escapeArgument', $args));
}
...
function createInkscapeShellFile(array $commands): string
{
    $text = '';
